            ...
            <div class="card-body">
                {{-- <form action="{{ route('products.update',$product->id) }}" method="POST"> --}}
                    <form method="POST" action="{{ route('products.update', ['product' => $product->id]) }}">
                        @csrf
                        @method('PUT') {{-- or use @method('PATCH') based on your form method --}}
                    ...
                    <div class="">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
                            <div class="form-group">
                                ...
                                <textarea id="myTextarea" class="form-control" name="detail">{{ $product->detail }}</textarea>
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
...
@endsection
@push('iconcss')
<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/spectrum@1.8.0/dist/spectrum.min.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.14.0/css/all.min.css" integrity="sha512-..." crossorigin="anonymous" />
@endpush

@push('tinymce')
    <script src="https://cdn.jsdelivr.net/npm/spectrum@1.8.0/dist/spectrum.min.js"></script>
        ...
        <script>
            tinymce.init({
                ...
                setup: function (editor) {
                    // Add Font Awesome icons to the toolbar
                    editor.ui.registry.addIcon('custom-icon', '<i class="fas fa-icon"></i>');
                    
                    // Add Color Picker to the toolbar
                    editor.ui.registry.addToggleButton('color-picker', {
                        icon: 'custom-icon',
                        tooltip: 'Color Picker',
                        onAction: function (_) {
                            // Implement color picker functionality
                            // You may need to adjust this part based on the color picker library you're using
                            // Example: spectrum.colorpicker(options);
                        }
                    });
                }
            });

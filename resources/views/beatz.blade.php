...
@section('content')
    ...
    <div class="articles">
        ...
        <div class="article-card">
            <div class="cover">
                ...
                <div class="overlay">
                    <a href="#" class="play-button track-list" data-track="{{ \Storage::url($beat->demo) }}"
                        data-poster="{{ $beat->image ? \Storage::url($beat->image) : '' }}" data-title="{{ $beat->title ?? '-' }}" data-singer="{{ $beat->artist ?? '-' }}">
                        <i class="icon-play"></i>
                    </a>
                </div>
            </div>
            ...
            <div class="details">
                ...
                @if ($beat->amount == 0)
                <a href="{{ route('beat.slug', ['slug' => urlencode($beat->slug)]) }}" class="btn buy-button">Download</a>
                @else
                <a href="{{ route('beat.slug', ['slug' => urlencode($beat->slug)]) }}" class="btn buy-button">Buy R{{ $beat->amount }}</a>
                @endif
    ...
    <div class="pagination">{{ $beats->links() }}</div>
@endsection
@section('head')
